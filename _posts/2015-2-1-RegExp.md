...
[正则表达式（RegExp）](https://msdn.microsoft.com/zh-cn/library/28hw3sce(v=vs.90).aspx)是一种文本模式，包括普通字符（例如，a 到 z 之间的字母）和特殊字符（元字符）。
...
式中，$X$ 为故障树中各基本事件。其中第 $i$ 个基本事件的结构重要度系数可以表示为

$$I_i=\frac{1}{k}\sum_{j=1}^m\frac{1}{R_j}$$

式中，$k$ 为故障树包含的最小割集集合数目，$m$ 为包含第 $i$ 个基本事件的最小割集集合数目，$R_j$ 为包含第 $i$ 个基本事件的第 $j$ 个最小割集集合中基本事件的数目。
